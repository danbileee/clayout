...
export default {
	async fetch(request, env, ctx) {
		...
		if (host.endsWith(DEFAULT_DOMAIN)) {
			...
			const reservedListRaw = await env.SITE_ROUTING_KV.get('reserved_subdomains', { type: 'json' });
			const reservedList = Array.isArray(reservedListRaw) && reservedListRaw.every(String) ? (reservedListRaw as string[]) : [];

			if (reservedList.map((s) => s.toLowerCase()).includes(subdomain.toLowerCase())) {
